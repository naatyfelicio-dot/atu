...
local function setEnabled(value)
    ...
    if value then
        ...
        StopDetection()
        stopStepConnection()
        -- RESTAURA o PhysicsRepRootPart (senão você não anda)
...
local GearBtn = createInstance("TextButton", {
...
local drag2, start2, pos2, dragInput2
...
connect(SetHeader.InputEnded, function(input)
    if input == dragInput2 or input.UserInputType == Enum.UserInputType.MouseButton1 then
        drag2 = false
        dragInput2 = nil
    end
end)
connect(UserInputService.InputChanged, function(input)
    if drag2 and (input.UserInputType == Enum.UserInputType.MouseMovement or input == dragInput2) then
